...
# %% auto 0
__all__ = ['EOI', 'get_eoi_patterns', 'colorize', 'plot_ptrn', 'plot_patterns', 'plot_puzzle', 'get_instrument_pattern',
           'add_hotlines']
...
def plot_ptrn(elem, y, ax, element_ptrns=None, color_select=None): 
    '''Low level plot element pattern at level `y` in axes `ax`.'''

    # this is a rather slow function
    # so better provide element_ptrns list 
    if element_ptrns == None: 
        element_ptrns = get_patterns()
...
def plot_patterns(ptrn_list=None, elements=None, instr_ptrn_from_datastack_file=None, color_select=None, ax=None): 
    ...
    if ptrn_list == None: 
        ptrn_list = get_patterns(elements=elements, tube_keV=40, color_select=color_select) 
...
def plot_puzzle(datastack_file, n, element_ptrns=None, elements=None, color_select=None, footspace=0.2): 
